...
have been converted with convert/convert_manifold_nest_flax.py
...
def _cfg32(url='', **kwargs):
    return {
        'url'        : url,
        'num_classes': 1000, 'input_size': (3, 32, 32),'pool_size': [14, 14],
        'crop_pct'   : .875, 'interpolation': 'bicubic', 'fixed_input_size': True,
        'mean'       : IMAGENET_DEFAULT_MEAN, 'std': IMAGENET_DEFAULT_STD,
        'first_conv' : 'patch_embed.proj', 'classifier': 'head',
        **kwargs
...
'manifold_nest_tiny_s16_32'    : _cfg32(),
    'jx_manifold_nest_base' : _cfg(
        url='https://github.com/rwightman/pytorch-image-models/releases/download/v0.1-vt3p-weights/jx_manifold_nest_base'
            '-8bc41011.pth'),
    'jx_manifold_nest_small': _cfg(
        url='https://github.com/rwightman/pytorch-image-models/releases/download/v0.1-vt3p-weights/jx_manifold_nest_small'
            '-422eaded.pth'),
    'jx_manifold_nest_tiny' : _cfg(
        url='https://github.com/rwightman/pytorch-image-models/releases/download/v0.1-vt3p-weights/jx_manifold_nest_tiny'
            '-e3428fb9.pth'),
}
...
class EuclRiemGrassAtt(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=True, attention_dropout=0.1, projection_dropout=0.1,
        ...
        self.conv_attn = nn.Sequential(
            nn.BatchNorm3d(2 * num_heads),
            nn.Conv3d(in_channels=2 * self.num_heads, out_channels=num_heads, kernel_size=(1, 1,1))
        )
    ...
    def forward(self, x):
       ...
       # dots = torch.matmul(qgr, kgr).unsqueeze(2)
        ...
        B,H,t1,t2,t3 = attn.shape
        #print(f' attn {attn.shape}  attn_riemmanian {attn_riemmanian.shape} ttn_grassmman   {attn_grassmman.shape }')
        attn_ = self.conv_attn(torch.cat((attn, attn_riemmanian), dim=1)).softmax(
            dim=-1)
        #
        #attn_ = (attn+attn_riemmanian).softmax(dim=-1)
        ...
        out = out.permute(0, 2, 3, 4, 1).reshape(B, T, N, C)
...
class ManifoldNest(nn.Module):
    ...
    def init_weights(self, mode=''):
        ...
        named_apply(partial(_init_manifold_nest_weights, head_bias=head_bias), self)
...
def _init_manifold_nest_weights(module: nn.Module, name: str = '', head_bias: float = 0.):
...
def _create_nest(variant, pretrained=False, default_cfg=None, **kwargs):
    ...
    model = build_model_with_cfg(
        ManifoldNest, variant, pretrained,
...
@register_model
def manifold_nest_base(pretrained=False, **kwargs):
    """ Nest-B @ 224x224
    """
    model_kwargs = dict(
        embed_dims=(128, 256, 512), num_heads=(4, 8, 16), depths=(2, 2, 20), **kwargs)
    model = _create_nest('manifold_nest_base', pretrained=pretrained, **model_kwargs)
    return model


@register_model
def manifold_nest_small(pretrained=False, **kwargs):
    """ Nest-S @ 224x224
    """
    model_kwargs = dict(embed_dims=(96, 192, 384), num_heads=(3, 6, 12), depths=(2, 2, 20), **kwargs)
    model = _create_nest('manifold_nest_small', pretrained=pretrained, **model_kwargs)
    return model


@register_model
def manifold_nest_tiny(pretrained=False, **kwargs):
    """ Nest-T @ 224x224
    """
    model_kwargs = dict(embed_dims=(96, 192, 384), num_heads=(3, 6, 12), depths=(2, 2, 8), **kwargs)
    model = _create_nest('manifold_nest_tiny', pretrained=pretrained, **model_kwargs)
    return model


@register_model
def manifold_nest_tiny_32(pretrained=False, **kwargs):
    """ Nest-T @ 224x224
    """
    model_kwargs = dict(embed_dims=(96, 192, 384), num_heads=(3, 6, 12), depths=(2, 2, 8), **kwargs)
...
@register_model
def jx_manifold_nest_base(pretrained=False, **kwargs):
    ...
    model = _create_nest('jx_manifold_nest_base', pretrained=pretrained, **model_kwargs)
    return model


@register_model
def jx_manifold_nest_small(pretrained=False, **kwargs):
    """ Nest-S @ 224x224, Pretrained weights converted from official Jax impl.
    """
    kwargs['pad_type'] = 'same'
    model_kwargs = dict(embed_dims=(96, 192, 384), num_heads=(3, 6, 12), depths=(2, 2, 20), **kwargs)
    model = _create_nest('jx_manifold_nest_small', pretrained=pretrained, **model_kwargs)
    return model


@register_model
def jx_manifold_nest_tiny(pretrained=False, **kwargs):
    """ Nest-T @ 224x224, Pretrained weights converted from official Jax impl.
    """
    kwargs['pad_type'] = 'same'
    model_kwargs = dict(embed_dims=(96, 192, 384), num_heads=(3, 6, 12), depths=(2, 2, 8), **kwargs)
    model = _create_nest('jx_manifold_nest_tiny', pretrained=pretrained, **model_kwargs)
    return model
