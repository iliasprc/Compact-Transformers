...
class Manifold_ConViT(nn.Module):
...
def _create_manifold_convit(variant, pretrained=False, **kwargs):
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for Vision Transformer models.')

    return build_model_with_cfg(
        Manifold_ConViT, variant, pretrained,
...
def manifold_convit_tiny(pretrained=False, **kwargs):
    model_args = dict(
        local_up_to_layer=10, locality_strength=1.0, embed_dim=48,
        num_heads=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model = _create_manifold_convit(variant='manifold_convit_tiny', pretrained=pretrained, **model_args)
    return model


@register_model
def manifold_convit_small(pretrained=False, **kwargs):
    model_args = dict(
        local_up_to_layer=10, locality_strength=1.0, embed_dim=48,
        num_heads=9, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model = _create_manifold_convit(variant='manifold_convit_small', pretrained=pretrained, **model_args)
    return model


@register_model
def manifold_convit_base(pretrained=False, **kwargs):
    model_args = dict(
        local_up_to_layer=10, locality_strength=1.0, embed_dim=48,
        num_heads=16, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model = _create_manifold_convit(variant='manifold_convit_base', pretrained=pretrained, **model_args)
    return model
