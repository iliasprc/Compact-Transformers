...
default_cfgs = {
    'T2t_vit_7_32': _cfg_32(),
    'T2t_vit_10_32': _cfg_32(),
    'T2t_vit_7': _cfg(),
    'T2t_vit_10': _cfg(),
    'T2t_vit_12': _cfg(),
    'T2t_vit_14': _cfg(),
...
class Attention_Token_transformer(nn.Module):
...
class Token_transformer(nn.Module):
    ...
    def __init__(self, dim, in_dim, num_heads, mlp_ratio=1., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention_Token_transformer(
...
@register_model
def t2t_vit_7_32(pretrained=False, **kwargs): # adopt performer for tokens to token
    if pretrained:
        kwargs.setdefault('qk_scale', 256 ** -0.5)
    model = T2T_ViT(tokens_type='performer', embed_dim=256, depth=7, num_heads=4, mlp_ratio=2., **kwargs)
    model.default_cfg = default_cfgs['T2t_vit_7_32']
    if pretrained:
        load_pretrained(
